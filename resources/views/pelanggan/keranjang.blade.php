...
<body class="bg-gray-100">
  ...
  <!-- Bungkus seluruh konten + bottom bar + modal dalam satu Alpine component -->
  <div x-data="keranjangApp()" x-init="init()" class="relative">
    ...
    <div class="max-w-5xl mx-auto p-6 bg-white rounded shadow pb-32 space-y-6">
      ...
      <div class="flex items-center justify-between mb-4">
        ...
        <a href="{{ route('pesan.index', ['nomor_meja' => $meja->nomor]) }}"
           class="border border-orange-500 text-orange-500 px-3 py-1 rounded hover:bg-orange-50 transition">
          + Tambah Item
        </a>
      </div>
            ...
            <div class="mt-1 flex items-center space-x-2">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-yellow-500 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M7 8h6m-6 4h6m2 2H5a2 2 0 01-2-2V4a2 2 0 012-2h7l5 5v7a2 2 0 01-2 2z"/>
              </svg>
              <input
                x-ref="catatanInput"
                type="text"
                x-model="catatan"
                @input.debounce.500ms="sendUpdate()"
            ...
            <div class="flex justify-between items-center mt-3">
              ...
              <div class="flex items-center space-x-2">
                <button @click="decrease()"
                        class="flex h-8 w-8 items-center justify-center rounded-full border border-gray-300 hover:bg-gray-100 transition">−</button>
      ...
      <!-- Empty State -->
      <div x-show="isEmpty" class="text-center py-12 text-gray-500 font-medium">
        Keranjang masih kosong.
      </div>

      <!-- Subtotal -->
      <div class="text-right" x-show="!isEmpty">
        <span class="text-gray-600">Subtotal:</span>
        <span class="text-xl font-bold text-gray-900" x-text="'Rp ' + total.toLocaleString()"></span>
      </div>

      <!-- Form Konfirmasi ➝ buka modal -->
      <form x-ref="confirmForm"
            action="{{ route('pesan.confirm') }}"
            method="POST"
            class="mt-6"
            x-show="!isEmpty">
        @csrf
        ...
        <button type="button"
                ...
                class="bg-green-600 text-white px-6 py-3 rounded-lg w-full hover:bg-green-700 transition">
          Konfirmasi Pesanan
        </button>
      </form>
    </div>
    ...
    <!-- Fixed Bottom Bar -->
    <div
         class="fixed bottom-0 left-0 right-0 bg-white border-t shadow-lg px-6 py-4 flex items-center justify-between z-40">
      <div class="text-sm">
    ...
    <div x-show="showModal" x-cloak
      ...
      <p class="text-center text-lg font-semibold mb-4">
        Apakah Anda yakin ingin konfirmasi pesanan?
      </p>
  ...
  <script>
    ...
    function itemRow(id, jumlahAwal, harga, catatanAwal) {
      return {
        id, jumlah: jumlahAwal, harga, catatan: catatanAwal,
